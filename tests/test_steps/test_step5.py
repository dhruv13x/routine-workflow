...
def test_generate_dumps_no_tool(mock_runner: Mock):
    ...
    mock_runner.logger.warning.assert_called_once_with('code-dump not found - skipping generation')
...
def test_generate_dumps_dry_run(mock_exists, mock_runner: Mock):
    ...
    mock_runner.config.code_dump_run_cmd = ['code-dump', 'batch', 'run']  # Set to avoid attr error
...
def test_generate_dumps_success(mock_exists, mock_run, mock_runner: Mock):
    ...
    mock_runner.config.code_dump_run_cmd = ['code-dump', 'batch', 'run']
    mock_exists.return_value = True
    ...
    mock_run.assert_called_once_with(
        mock_runner, 'Batch generate code dumps', ['code-dump', 'batch', 'run', '-nd', '-y'],
...
def test_generate_dumps_failure(mock_exists, mock_run, mock_runner: Mock):
    ...
    mock_run.assert_called_once_with(
        mock_runner, 'Batch generate code dumps', ['code-dump', 'batch', 'run', '-nd'],
        cwd=mock_runner.config.project_root, timeout=600.0, fatal=False
    )
...
def test_generate_dumps_cmd_fallback(mock_exists, mock_run, mock_runner: Mock):
    """Test fallback cmd if code_dump_run_cmd not set."""
    mock_runner.config.dry_run = False
    mock_runner.config.auto_yes = False
    mock_runner.config.project_root = Path('/tmp/project')
    mock_runner.config.code_dump_run_cmd = None  # Trigger fallback
    mock_exists.return_value = True
    mock_run.return_value = True
    ...
    mock_run.assert_called_once_with(
        mock_runner, 'Batch generate code dumps', ['code-dump', 'batch', 'run', '--dirs', '., packages, packages/platform_core, packages/telethon_adapter_kit, services, services/forwarder_bot', '-nd'],
        cwd=mock_runner.config.project_root, timeout=600.0, fatal=False
    )
...
def test_generate_dumps_fallback_dry_run(mock_exists, mock_runner: Mock):
    ...
    mock_runner.config.dry_run = True
    mock_runner.config.code_dump_run_cmd = None
    mock_exists.return_value = True
...
def test_generate_dumps_fallback_failure(mock_exists, mock_run, mock_runner: Mock):
    ...
    mock_run.assert_called_once_with(
        mock_runner, 'Batch generate code dumps', ['code-dump', 'batch', 'run', '--dirs', '., packages, packages/platform_core, packages/telethon_adapter_kit, services, services/forwarder_bot', '-nd', '-y'],
        cwd=mock_runner.config.project_root, timeout=600.0, fatal=False
    )
    mock_runner.logger.warning.assert_called_with('Code-dump generation failed or skipped')