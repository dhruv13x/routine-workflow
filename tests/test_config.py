...
def test_from_args_with_defaults(mock_resolve, mock_mkdir: Mock, mock_cpu: Mock, mock_defaults: Mock, mock_args: Mock):
    ...
    mock_args.code_dump_script = Path('/tmp/dump.sh')
    ...
    mock_args.code_dump_run_cmd = None  # No override
    ...
    assert cfg.code_dump_script == mock_args.code_dump_script
    assert cfg.exclude_patterns == ['default/*']
    assert cfg.code_dump_run_cmd == ['code-dump', 'batch', 'run', '--dirs', '., packages, packages/platform_core, packages/telethon_adapter_kit, services, services/forwarder_bot']
    assert cfg.code_dump_clean_cmd == ['code-dump', 'batch', 'clean']
...
def test_from_args_with_override(mock_resolve, mock_mkdir: Mock, mock_defaults: Mock, mock_args: Mock):
    ...
    mock_args.code_dump_script = Path('.')
    ...
    mock_args.code_dump_run_cmd = ['code-dump', 'batch', 'run', '--dirs', 'custom']
    ...
    assert cfg.code_dump_run_cmd == mock_args.code_dump_run_cmd
...
def test_max_workers_override(mock_resolve, mock_cpu: Mock, mock_args: Mock):
    ...
    mock_args.workers = 2
    mock_args.workflow_timeout = 0
    mock_args.code_dump_run_cmd = None
